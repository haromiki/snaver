...
## **통계 그래프 기간별 적응형 그룹화 기준** (2025-08-27 추가)
**문제점**: 시간대 오류, 과도한 세밀함, 기간 필터링 오류

**해결책**: 기간별 적응형 그룹화 기준 적용
- **1개월 이하**: 일별 평균 (하루에 여러 검색이 있어도 하나로 통합)
- **3개월 이하**: 주별 평균 (월요일 기준으로 주간 데이터 통합)
- **6개월 이하**: 2주별 평균 (격주 단위 데이터 통합)
- **1년 이하**: 월별 평균 (월 단위 데이터 통합)
- **2년**: 분기별 평균 (3개월 단위 데이터 통합)

**목표**: 의미 있는 트렌드만 표시, 그래프 가독성 향상, 기간에 맞는 적절한 해상도 제공

**시간대 처리**: 모든 데이터는 한국 표준시(KST) 기준으로 변환 후 그룹화
**기간 필터링**: 선택한 조회 기간 범위 내의 데이터만 정확히 필터링
